...
## Como conferir que está salvando no Neon
- **Logs do Render** (na inicialização): `[db] Conectado ao Postgres: host=... banco=...` e depois `[app] ... tabelas prontas no Neon`.
  A cada entrada/saída no totem aparece `[ponto] entrada salva no Postgres`.
- **Navegador:** abra `https://SEU-SITE.onrender.com/api/health` → deve mostrar `{"ok":true,"banco":"neondb",...}` com a contagem de registros.
- **Neon:** Console → *Tables* (ou *SQL Editor*): `SELECT * FROM employees ORDER BY id DESC LIMIT 10;`

## Novo colaborador no totem
Na tela "Colaborador Tenda", quem não está na lista pode tocar em **+ Novo colaborador** (ou digitar uma matrícula
desconhecida, que abre o campo de nome automaticamente) e se cadastrar. O nome é salvo em MAIÚSCULAS na tabela
`colaboradores` (`origem = 'totem'`) e o ponto já é registrado. Esses cadastros não são apagados quando a lista
`data/colaboradores.json` é sincronizada.
