  ...
  try {
    if (tipo === 'tenda') {
      ...
      if (c.rowCount) name = c.rows[0].nome;
        ...
        if (nomes.size !== 1) return res.status(404).json({ error: 'matricula' });
        name = r.rows[0].nome;
      }
    } else {
...
async function loadColaboradores() {
  ...
  try {
    ...
    for (const c of list) {
      await client.query(
        `INSERT INTO colaboradores (chapa, nome, setor, funcao) VALUES ($1,$2,$3,$4)
         ON CONFLICT (chapa) DO UPDATE SET nome=$2, setor=$3, funcao=$4`,
        [String(c.chapa).trim(), c.nome, c.setor || '', c.funcao || '']
      );
    }
    await client.query('DELETE FROM colaboradores WHERE NOT (chapa = ANY($1::text[]))', [list.map((c) => String(c.chapa).trim())]);
    await client.query('COMMIT');
